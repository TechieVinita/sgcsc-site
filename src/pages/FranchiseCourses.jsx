...
export default function FranchiseCourses() {
  ...
  // Load franchise profile to know which courses belong to this franchise
  useEffect(() => {
  ...
  // FIX: defined isOwnCourse — was called but never defined
  const isOwnCourse = (course) => {
    if (!franchiseId) return false;
    const createdBy =
      typeof course.createdBy === "object"
        ? (course.createdBy?._id || course.createdBy?.id || "").toString()
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
          ...
          <small className="text-muted">
            View courses created by you and admin-created courses available for
            your franchise.
          </small>
        </div>
        <button
